...
class DeepVSExperiment:
    ...
    def run(self, datasetPath, proteinsNames_training, proteinsNames_test, proteinRestrictions):
        ...
        print "Loading data ..."
        molName_training, molClass_training, molData_training = load_data(datasetPath, self.kc, self.kp, proteinsNames_training, rng, randomize = True)
        ...
        molName_test, molClass_test, molData_test = load_data(datasetPath, self.kc, self.kp, proteinsNames_test, rng, randomize = False)
        ...
        for k in xrange(self.num_epochs):
            print "Ep: %d, AUC: %.4f -"%(k+1, self._aucSumByEpoch[k]/self._numProteinProcessed),
        print " "
        for k in xrange(self.num_epochs):
            print "Ep: %d, EF 2%%: %.4f -"%(k+1, self._Ef2SumByEpoch[k]/self._numProteinProcessed),
        print " "
        for k in xrange(self.num_epochs):
            print "Ep: %d, EF 20%%: %.4f -"%(k+1, self._Ef20SumByEpoch[k]/self._numProteinProcessed),
        print " "
        for k in xrange(self.num_epochs):
            print "Ep: %d, EF Max: %.4f -"%(k+1, self._EfMaxSumByEpoch[k]/self._numProteinProcessed),
        print " "
        sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    proteinNames = ['ace', 'ache', 'ada', 'alr2', 'ampc', 'ar', 'cdk2','comt', 'cox1', 
